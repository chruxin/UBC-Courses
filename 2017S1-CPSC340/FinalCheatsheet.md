...
| **RBFs**                          | Z: O(n^2d), Z^TZ: O(n^3), inverse Z^TZ: O(n^3), training: O(n^2d+n^3), prediction: O(tnd) | Yes                 |
| **Ordinary Least Squares**        | X^TX: O(nd^2), inverse X^TX: O(d^3), training: O(nd^2+d^3), prediction: O(td) | No                  |
| **Regularized Least Squares**     |                                          | Yes                 |
| **Kernel Trick with Polynomials** | Training: O(n^2d+n^3) Testing: O(ndt)    |                     |
| **Naive Bayes**                   | Prediction: O(tdk)                       |                     |
